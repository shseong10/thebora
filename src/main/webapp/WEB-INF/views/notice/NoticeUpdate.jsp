    ...
    <form action="/notice/update" method="post" enctype="multipart/form-data">
        <table class="table table-dark table-striped" style="width: 800px; margin: auto; text-align: center;">
            <tr>
                <td>글번호</td>
                <td>
                    <input type="text" name="n_num" value="${nDto.n_num}" readonly>
                </td>
            </tr>
            <tr>
                <td>구분</td>
                <td>
                    <select name="n_kind" id="kind">
                        <option value="공지" name="n_kind">공지</option>
                        <option value="광고" name="n_kind">광고</option>
                    </select>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <c:if test="${!empty nDto.nfList}">
                    <c:forEach var="files" items="${nDto.nfList}">
                        <td>${files.nf_sysFileName} <i>${files.nf_oriFileName}</i></td>
                    </c:forEach>
                </c:if>
            </tr>
            <tr>
